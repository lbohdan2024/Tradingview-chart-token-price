...
(self.webpackChunktradingview = self.webpackChunktradingview || []).push([
  [507],
  {
    ...
    56208: (t, e, i) => {
      ...
      const f = (0, h.getVolumeFormatter)();
      class p {
        constructor(t) {
          this._data = t;
        }
          ...
          for (const s of i.histograms) {
            ...
            for (const a of s.bars) {
              ...
              const l = _(s.xRange, o, e.mediaSize.width),
                { xBasePoint: h, sign: c, xRangeWidth: u } = l,
                f = Math.max(
              ...
              for (let e = 0; e < a.subBarValues.length; e++) {
                ...
                if (
                  ...
                  return new d.HitTestResult(d.HitTarget.Regular);
              }
            }
          }
          return null;
        ...
        draw(t, e) {
          ...
          t.save(),
            i.histograms.forEach((a) => {
              ...
              a.bars.forEach((t) => {
                const e = t.subBarValues.reduce((t, e) => t + e);
                (l = Math.max(l, e)), (d += t.height);
              });
              const h = d / a.bars.length,
                c =
                  ((u = h),
                  (f = r),
                  Math.floor(u * f) >= 1 * f ? Math.floor(f) : 0);
              var u, f;
              const p = [];
              if (
                (a.bars.forEach((s) => {
                  const d = i.styles[s.styleId];
                  if (!d.visible) return;
                  if (d.showValues && d.addToTotalValue)
                    for (let t = 0; t < s.subBarValues.length; t++)
                      o[t] = (o[t] || 0) + s.subBarValues[t];
                  const h = _(a.xRange, d, e.mediaSize.width),
                    { xBasePoint: u, sign: f, xRangeWidth: m } = h;
                  if (s.y > e.mediaSize.height || s.y + s.height < 0) return;
                  const x = Math.max(
                    (d.percentWidth * m) / 100 - s.subBarValues.length,
                    0,
                  );
                  let y = u;
                  for (let e = 0; e < s.subBarValues.length; e++) {
                    const i = s.y,
                      a = s.y + s.height,
                      o = y,
                      h = o + f * ((x * s.subBarValues[e]) / l);
                    if (Math.abs(h - o) < 0.5) continue;
                    (y = h), t.beginPath(), (t.fillStyle = d.colors[e]);
                    const u = Math.round(o * r),
                      p = Math.round(i * n),
                      _ = Math.round(h * r) - u,
                      v = Math.max(Math.round(a * n) - p - c, 1);
                    t.rect(u, p, _, v), t.fill();
                  }
                  if (!d.showValues) return;
                  const w = v(s.displayedValues, d.direction),
                    R = g(m, s.y, s.height, h, d, w);
                  p.push(R);
                }),
                o.length > 0)
              ) {
                const t = i.styles[a.bars[0].styleId],
                  ...
                  d = g(r.xRangeWidth, l.y + l.height, h, r, t, n);
                (d.color = (0, s.shiftColor)(d.color, 1.5)), p.push(d);
              }
              const x = Math.min(...p.map((t) => t.fontSize));
              if (x >= 7.5) for (const i of p) (i.fontSize = x), m(t, e, i);
            }),
            t.restore();
        }
      }
      ...
      function v(t, e) {
        return (
          ...
          t.map((t) => f.format(t)).join("x")
        );
      }
      function g(t, e, i, s, r, a) {
        const { fontSize: l, verticalOffset: d } = (function (t, e, i) {
            return {
              fontSize: Math.min(
          ...
          h = r.direction === n.HHistDirection.LeftToRight ? "left" : "right",
      ...
      class w {
        constructor(t, e, i, s, r = !1, n = !1) {
          (this._invalidated = !0),
            ...
            (this._hhistRenderer = new p(this._rendererData));
        }
        update() {
          this._invalidated = !0;
        }
          ...
          const i = (0, r.ensureDefined)(this._provider.graphicsInfo().hhists),
            ...
            o =
              (null ===
                ...
                ? void 0
                : e.value()) === n.HHistVolumeMode.Delta;
          for (const t of s) {
            const e = (0, r.ensureDefined)(a.childs()[t]).childs(),
        ...
        _updateDataForRenderers(t, e, i) {
          ...
          const d =
              (null ===
                (a =
                  ...
                  void 0 === s
                    ? void 0
                    : s.child("volume")) || void 0 === a
                ? void 0
                : a.value()) === n.HHistVolumeMode.Delta,
            h = (function (t, e, i) {
              const s = e.barBorders(t.firstBarTime),
                r = e.barBorders(t.lastBarTime);
              return i ? u(s.left, r.right) : u(s.center, r.center);
          ...
          t.forEach((t) => {
            ...
            this._rendererData.histograms.push({
              xRange: h,
              yRange: u(f[0].y, f[f.length - 1].y + f[f.length - 1].height),
              bars: f,
            });
        }
      }
    ...
    95258: (t, e, i) => {
      ...
      class o {
        ...
        _updateViewInternal() {
          ...
          i.forEach((i, r) => {
            const d = (0, s.ensureDefined)(l.childs()[r]).childs();
            d.visible.value() &&
              i.forEach((i) => {
                ...
                if (!i.extendLeft) {
                  ...
                  h = this._extendToBarsEndings ? t.left : t.center;
                }
                if (!i.extendRight) {
          ...
          r.forEach((r, a) => {
            ...
            c.visible.value() &&
              r.forEach((i) => {
                const r = i.index;
                if (r < l || d < r) return;
                const { left: a, center: h, right: f } = e.barBorders(r);
                let p;
                switch (u.halign) {
                  case n.HAlign.Left:
                    p = a;
